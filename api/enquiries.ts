...
function validate(raw: EnquiryInput) {
  ...
  if (data.kind === 'call_request' && !data.phone) errors.phone = 'Add a phone number so we can confirm your call.';
  if (data.kind !== 'call_request' && !data.email && !data.phone) errors.contact = 'Add an email address or WhatsApp number so we can reply.';
  if (data.email && !validEmail(data.email)) errors.email = 'Enter a valid email address.';
  if (data.phone && !validWhatsApp(data.phone)) {
    errors[data.kind === 'call_request' ? 'phone' : 'whatsapp'] = data.kind === 'call_request'
  ...
  if (data.kind !== 'call_request' && data.projectGoal.length < 12) errors.projectGoal = 'Tell us a little more about the result you need.';
...
function emailHtml(data: ReturnType<typeof validate>['data']) {
  const rows = labelledFields(data)
    .map(([label, content]) => `<tr><th style="padding:10px 12px;text-align:left;vertical-align:top;border-bottom:1px solid #e5e7eb">${escapeHtml(label)}</th><td style="padding:10px 12px;white-space:pre-wrap;border-bottom:1px solid #e5e7eb">${escapeHtml(content)}</td></tr>`)
    .join('');
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error } = await resend.emails.send(
      {
        from,
        to: data.kind === 'call_request' ? 'contact@webstell-studio.com' : teamEmail,
        replyTo: data.email || undefined,
        subject: data.kind === 'call_request' ? `Call request from ${data.name}` : `New WEBSTELL brief from ${data.name}`,
        html: emailHtml(data),
        attachments: data.attachment ? [{ filename: data.attachment.filename, content: data.attachment.content }] : undefined,
        text: labelledFields(data).map(([label, content]) => `${label}: ${content}`).join('\n\n'),
      },
      { idempotencyKey: `webstell-enquiry-${data.submissionId}` },
    );

    if (error) {
      console.error('Resend enquiry delivery failed:', error.name, error.message);
      return Response.json({ ok: false, message: 'We could not send this right now. Please try again or message us on WhatsApp.' }, { status: 502 });
    }

    return Response.json({ ok: true, customerCopySent: false });
  } catch (error) {
    console.error('Enquiry email configuration failed:', error instanceof Error ? error.message : 'Unknown error');
