...
<section className="case-studies case-studies-international" aria-labelledby="case-studies-title"><div className="case-studies-heading wrap"><span className="case-badge"><span aria-hidden="true">✦</span> International projects</span><h2 id="case-studies-title">Digital experiences<br/>built worldwide.</h2><p>WEBSTELL team members have contributed to these digital experiences as part of international partner and client teams.</p></div><div className="case-grid wrap">{featuredProjects.map((project,i)=><article className={'case-card case-card-'+(i+1)+' case-tone-'+project.tone} key={project.title}><div className="case-copy"><div className="case-meta-row"><span className="case-index">{project.number}</span><span className="case-location">{project.location}</span></div><h3>{project.title}</h3><p>{project.description}</p><a className="case-project-link" href={project.url} target="_blank" rel="noreferrer">View project <span aria-hidden="true">→</span></a></div><a className="case-visual" href={project.url} target="_blank" rel="noreferrer" aria-label={'View the '+project.title+' project'}><img src={project.image} alt={project.title+' project preview'} loading={i===0?'eager':'lazy'}/><span className="case-visual-open" aria-hidden="true"><b>View project</b><i>↗</i></span></a></article>)}</div><div className="case-action"><a href="/projects">See all projects <span aria-hidden="true">↗</span></a></div></section>
 ...
 <section className="insights" id="insights" aria-labelledby="why-webstell-title"><div className="insights-marquee" aria-label="Why WEBSTELL"><div className="insights-track"><span>Why WEBSTELL <i aria-hidden="true">✳</i> Why WEBSTELL <i aria-hidden="true">✳</i></span><span aria-hidden="true">Why WEBSTELL <i>✳</i> Why WEBSTELL <i>✳</i></span></div></div><div className="insights-inner wrap"><div className="insights-lead"><p className="insights-kicker">ABOUT WEBSTELL</p><h2 id="why-webstell-title">Built for businesses that refuse to blend in.</h2><p className="insights-summary">WEBSTELL is a design and technology studio for ambitious businesses. We turn good ideas into distinctive websites, digital products and smarter systems that make the next move easier.</p><a className="all-articles" href="#contact"><span aria-hidden="true" data-hover-label="Start your project">↗</span> Start your project</a><div className="studio-email-row"><a className="studio-email" href="mailto:contact@webstell-studio.com">contact@webstell-studio.com</a><button type="button" className="copy-email" onClick={copyStudioEmail} aria-live="polite">{emailCopied?'Copied':'Copy email'}</button></div></div><div className="insight-grid" id="insight-grid"><div className="insight-image insight-image-red"><img src="/assets/insights-red.avif" alt="WEBSTELL design direction" loading="lazy"/></div><article className="insight-card featured"><span>{whyWebstellCards[0].eyebrow}</span><h3>{whyWebstellCards[0].title}</h3><p>{whyWebstellCards[0].text}</p><footer><small>WEBSTELL<br/><b>Design and technology studio</b></small><button onClick={()=>setContact(true)} aria-label="Start your project">↗</button></footer></article><article className="insight-card"><span>{whyWebstellCards[1].eyebrow}</span><h3>{whyWebstellCards[1].title}</h3><p>{whyWebstellCards[1].text}</p><footer><small>01 — 03<br/><b>What we provide</b></small><button onClick={()=>setContact(true)} aria-label="Start your project">↗</button></footer></article><div className="insight-image insight-image-blue"><img src="/assets/insights-blue.avif" alt="WEBSTELL digital product direction" loading="lazy"/></div><article className="insight-card wide"><span>{whyWebstellCards[2].eyebrow}</span><h3>{whyWebstellCards[2].title}</h3><p>{whyWebstellCards[2].text}</p><footer><small>01 Find focus · 02 Build with intent · 03 Keep moving</small><button onClick={()=>setContact(true)} aria-label="Start your project">↗</button></footer></article></div></div></section>
 <ContactSection/>
 </main>
