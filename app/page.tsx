...
export default function Home() {
 ...
 return <>
 <header className="header wrap"><a className="logo" href="#" aria-label="WEBSTHAL home">WEBSTHAL</a><button className="menu-button" onClick={()=>setMenu(!menu)} aria-expanded={menu} aria-controls="navigation">{menu?'Close':'Menu'} <span aria-hidden="true">{menu?'−':'☰'}</span></button><nav id="navigation" className={menu?'open':''}>{[['Services','services'],['Projects','projects'],['About','about'],['FAQ','faq'],['Contacts','contact']].map(([label,id])=><a key={id} className={id==='contact'?'pill':''} href={'#'+id} onClick={()=>setMenu(false)}>{label}</a>)}</nav></header>
 <main>
